...
- ✓ BugReportButton = microport-ui's `BugReportLauncher` (portalled to body at `bottom-20 md:bottom-4`, drop-zone screenshot form) + the app's `submit` transport, every authed user, POSTs to own `/api/bug-reports` (which signs + forwards to sp). → `bug-report-fanout`, `feedback_helpbutton_inline_zindex`
- ☐ **Every hoisted microport-ui shell still needs three things FROM the app** — the shell owns the DOM, so nothing below is optional. `BugReportButton.tsx` is the worked example. (a) a translated close label (`labels.close`, or `closeLabel` on `Dialog`/`ProfileModal`) — without it the ✕ falls back to a hardcoded English `aria-label="Close"`, a live zh/fr regression; (b) `slotProps` carrying this app's `testId(NS, ...)` spreads, or e2e loses every selector inside the shell; (c) `capturePageUrl="href"` on the bug report, because the payload default is `pathname + search` and the queue wants the absolute URL. All three are microport-ui ≥0.60.1. → `reference_testid_naming_convention`
...
- ✓ **`bg-overlay` utility** — same block also defines `--overlay:rgba(0,0,0,0.55)` (mapped via `--color-overlay:var(--overlay)` in `@theme`), matching the fleet's modal-scrim convention (salesport/opsport/reviewport/execport/clinicport). Use `bg-overlay` on the backdrop div of any modal/dialog/sheet instead of `bg-black/50` or an inline `rgba()` style (microport-ui's `Dialog` already handles its own scrim).
...
- ☐ **MANUAL: add BOTH new task roles to `GitHubActionsDeployRole`'s `EcrEcsDeploy` inline policy `iam:PassRole` resource list.** This template's ECS step re-pins to the latest task-def revision (`--task-definition "$LATEST_TD"`) so a task-def CONFIG change is actually adopted — a genuine fix for silently-stranded env/secret changes. **But re-pinning requires `iam:PassRole` on the task role, and the bare `--force-new-deployment` the 8 older satellites still use does NOT.** So the shared deploy policy was never granted PassRole for *any* `-dev-task-role`, and the gap is invisible until a repo adopts this file. It fails at the very last step, after a full successful image build and push:
  ```
  AccessDeniedException … not authorized to perform: iam:PassRole on resource:
  arn:aws:iam::682624892433:role/<slug>-dev-task-role
  ```
  Fixed fleet-wide 2026-08-23 by adding all nine `*-dev-task-role` ARNs (the list previously held only the nine **prod** `*-task-role`s), so a new mint now only needs its own two roles appended. → `bugfix_finport_dev_deploy_never_worked_2026-08-23`
- ☐ **MANUAL: confirm `API_URL` in BOTH `deploy.yml` and `deploy-dev.yml` names THIS app's API port.** The web container reaches the API over `localhost` inside the task, so a mis-substituted placeholder points the web build at another satellite's port and nothing catches it — finport shipped with `4100` (EngagePort's) against its own `4005` in both workflows, latent because the deploy had never once run. Since 2026-09-13 every port site is the `__API_PORT__` token, stamped from `config.apiPort`, and `scaffold.mjs` refuses a port already in the fleet list. That list is hardcoded until W1.1's `fleet.json`, so still cross-check it against `deploy-local`'s app table, and add the new app's port to both.
- ✓ **Prod deploy is CI-GATED by sha** — `deploy.yml` is `workflow_dispatch` (fleet-wide since 2026-09-01) and refuses unless the branch is main/master, the `sha` input is a full 40-hex id on that branch, and CI concluded `success` for that exact commit (`skip_ci_check` overrides, loudly). `deploy-local <app> prod` runs the same CI check. → `feedback_deploy_does_not_gate_on_ci`, `feedback_ecs_gha_stale_image`
- ✓ **Build credentials never enter an image** — `NODE_AUTH_TOKEN` / `SENTRY_AUTH_TOKEN` are BuildKit secret mounts (`--secret id=npm_token|sentry_token,env=…`), and CI's `build-secrets` job (`scripts/check-build-secrets.sh`) fails on any credential `ENV`/`ARG`/`--build-arg`. → `incident_build_token_leak_2026-09-06`
