...
- ✓ **`bg-overlay` utility** — same block also defines `--overlay:rgba(0,0,0,0.55)` (mapped via `--color-overlay:var(--overlay)` in `@theme`), matching the fleet's modal-scrim convention (salesport/opsport/reviewport/execport/clinicport). Use `bg-overlay` on the backdrop div of any modal/dialog/sheet instead of `bg-black/50` or an inline `rgba()` style — BugReportButton's scrim already uses it.
