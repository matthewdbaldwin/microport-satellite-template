...
- ✓ Outbound (bug reports) = **synchronous signed POST** to salesport `/api/bug-reports/cross-app` (`src/routes/bugReports.js`, `signWebhookBody`, `x-bugreport-signature`, 10s timeout, `BugReportCrossApp` contract validated on send). NOT a durable outbox — the scaffold's never-drained outbox is a known trap. → `feedback_scaffold_bug_report_fleet_pattern`
